...
module sui::crypto_tests {
    ...
    fun test_preqpare_verifying_key() {
        ...
        let arr = zkp::pvk_to_bytes(zkp::prepare_verifying_key(&vk));
    ...
    fun test_prepare_verifying_key_invalid() {
        let invalid_vk = x"";
        zkp::prepare_verifying_key(&invalid_vk);
    }
    ...
    fun test_verify_groth_16_proof() {
        ...
        let pvk = zkp::pvk_from_bytes(vk_bytes, alpha_bytes, gamma_bytes, delta_bytes);
        
        let inputs_bytes = x"4af76d91d4bc9a3973c15e3aeb574f0f64547b838f950af35db97f0705c4214b";
        let inputs = zkp::public_proof_inputs_from_bytes(inputs_bytes);
        ...
        let proof = zkp::proof_points_from_bytes(proof_bytes);
        
        assert!(zkp::verify_groth16_proof(pvk, inputs, proof) == true, 0);

        // Invalid prepared verifying key.
        vector::pop_back(&mut vk_bytes);
        let invalid_pvk = zkp::pvk_from_bytes(vk_bytes, alpha_bytes, gamma_bytes, delta_bytes);
        assert!(zkp::verify_groth16_proof(invalid_pvk, inputs, proof) == false, 0);

        // Invalid public inputs bytes.
        let invalid_inputs = zkp::public_proof_inputs_from_bytes(x"cf");
        assert!(zkp::verify_groth16_proof(pvk, invalid_inputs, proof) == false, 0);

        // Invalid proof bytes.
        let invalid_proof = zkp::proof_points_from_bytes(x"4a");
        assert!(zkp::verify_groth16_proof(pvk, inputs, invalid_proof) == false, 0);
    }
}
